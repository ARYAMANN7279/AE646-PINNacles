...
Samples with relative $L_2 > 0.2$ (approximately the top 5\% of test errors) share a common feature: the $\kappa$ field has a large number of distinct connected regions with sharp interfaces, creating rapid spatial transitions in $\nabla u$. These transitions require high-frequency Fourier modes to represent accurately --- exactly the modes discarded by the 12-mode FNO. Increasing the mode count and depth (FNO improved: modes=20, 6 blocks) helps most on the easy and typical cases --- the minimum error falls from 0.0155 to 0.0106 and the median from 0.0398 to 0.0307 --- but barely moves the worst case (max error 0.347 vs.\ 0.354), confirming that the hardest multi-region fields are limited by something other than raw mode count.
