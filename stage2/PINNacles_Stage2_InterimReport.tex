...
\end{verbatim}
\textbf{Platform and notebook.} During development each step (data preparation, training, evaluation, dataset analysis, ablations) was a separate Python script. We later combined them into one Jupyter notebook, \texttt{PINNacles\_Stage2\_Notebook.ipynb}, included in the code archive, which runs the whole pipeline and shows the results inline.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Run-to-run variance.} The first training runs did not fix the model-initialisation seed; re-training the identical MLP gives errors differing by $\sim0.004$ (0.0820 reported vs.\ 0.0857 in Section~\ref{sec:mlpabl}), which we treat as the noise floor: only larger gaps are interpreted. Training now seeds Python, NumPy and PyTorch, and the preprocessing ablation uses 3 fixed seeds per variant.
\end{enumerate}
