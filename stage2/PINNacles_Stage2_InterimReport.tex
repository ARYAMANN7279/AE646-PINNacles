...
\begin{figure}[H]
\centering
\includegraphics[width=0.5\textwidth]{figures/fig8_eda_dataset.pdf}
...
\begin{table}[H]
\centering
\caption{MLP preprocessing ablation: test mean relative $L_2$ (200 samples), mean $\pm$ std over 3 seeds.}
\label{tab:preabl}
\begin{tabular}{lr}
\toprule
...
\begin{figure}[H]
\centering
\includegraphics[width=0.42\textwidth]{figures/fig10_preprocess_ablation.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.42\textwidth]{figures/fig7_mlp_ablation.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.46\textwidth]{figures/fig1_error_histogram.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.66\textwidth]{figures/fig_sample_predictions.png}
...
\end{verbatim}
\textbf{Platform and notebook.} During development each step (data preparation, training, evaluation, dataset analysis, ablations) was a separate Python script. We later combined them into one Jupyter notebook, \texttt{PINNacles\_Stage2\_Notebook.ipynb}, included in the code archive, which runs the whole pipeline and shows the results inline. Development and unit tests ran on a MacBook (macOS, Apple Silicon); training, the ablations and the full notebook run on a GPU workstation (NVIDIA RTX PRO 6000, CUDA). The notebook reproduces the reported results closely (FNO 0.0528, MLP 0.0840, against 0.0521 and 0.0820 above) and all 16 unit tests pass.
...
\begin{enumerate}[nosep]
  \item \textbf{Metric convention.} Relative $L_2$ must be computed in physical units: on standardised fields the subtracted mean changes $\|u\|_2$, so the number is not comparable with the literature. All reported errors are de-normalised, and the metric is unit-tested.
  ...
  \item \textbf{Compute constraints.} Loading the 1.3\,GB raw file and training the MLP and the FNO (plus the ablations) was moved to the lab GPU machine.
  \item \textbf{Run-to-run variance.} The first training runs (the reported \texttt{run\_001}/\texttt{run\_002}) did not fix the model-initialisation seed; re-training the identical MLP gives errors differing by $\sim0.004$ (0.0820 reported vs.\ 0.0857 in Section~\ref{sec:mlpabl}), which we treat as the noise floor: only larger gaps are interpreted. Training now seeds Python, NumPy and PyTorch, and the preprocessing ablation uses 3 fixed seeds per variant.
\end{enumerate}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Final deliverables.} Final report, final presentation, and viva.
\end{enumerate}
