...
% Build (from stage3/):  tectonic PINNacles_Stage3_FinalPresentation.tex
% -> PINNacles_Stage3_FinalPresentation.pdf (the submitted file)
...
\end{columns}
\vfill
\footnotesize Training: AdamW (peak $10^{-3}$, per-step cosine cycling to 0), batch 16, weight decay $10^{-4}$, MSE loss on standardised pressure. 100 epochs (200 for FNO improved); the lowest-validation-error checkpoint is kept (no early stopping). Metric reported in physical (denormalised) units.
\end{frame}
...
\vfill
\begin{itemize}\footnotesize
  \item FNO original: \textbf{36\% lower mean error than MLP with $9\times$ fewer parameters}.
...
\includegraphics[width=\linewidth]{figures/fig1_error_histogram.pdf}
\end{column}
\begin{column}{0.42\textwidth}
\includegraphics[width=\linewidth]{figures/fig2_efficiency_scatter.pdf}
\end{column}
\end{columns}
\vfill
...
\begin{columns}
\begin{column}{0.5\textwidth}
\includegraphics[width=\linewidth]{figures/fig3_superres.pdf}
\end{column}
\begin{column}{0.5\textwidth}
\footnotesize
\begin{table}
...
\includegraphics[width=\linewidth]{figures/fig4_speed.pdf}
\end{column}
...
\includegraphics[width=\linewidth]{figures/fig5_components.pdf}
\end{column}
...
\begin{itemize}
  \item FNO beats the MLP baseline: \textbf{36\% lower error, $9\times$ fewer parameters}, zero-shot to $2\times$ resolution ($+14\%$), $>1000\times$ faster than FDM.
